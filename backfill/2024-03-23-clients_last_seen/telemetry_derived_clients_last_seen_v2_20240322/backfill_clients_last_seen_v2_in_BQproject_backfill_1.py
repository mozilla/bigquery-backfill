...
PARTITION_QUERY = """
    WITH _current AS (
      SELECT
        ...
        mozdata.udf.days_since_created_profile_as_28_bits(
    ...
    _previous AS (
      ...
      WHERE
        ...
        AND mozdata.udf.shift_28_bits_one_day(days_seen_bits) > 0
    )
    --
    SELECT
      ...
      IF(_current.client_id IS NOT NULL, _current, _previous).* REPLACE (
        mozdata.udf.combine_adjacent_days_28_bits(
        ...
        mozdata.udf.coalesce_adjacent_days_28_bits(
          _previous.days_created_profile_bits,
          _current.days_created_profile_bits
        ) AS days_created_profile_bits,
        mozdata.udf.combine_experiment_days(
...
parser.add_argument(
    "--project_id",
    "--project-id",
    default="moz-fx-data-backfill-1",
...
def _backfill_staging_table(client, job_config, project_id, dataset, destination_table, bigquery_schema, submission_date, sample_id):
    ...
    arguments = (
            ['query', '--use_legacy_sql=false', '--replace', f'--project_id={project_id}',
             '--format=none']
