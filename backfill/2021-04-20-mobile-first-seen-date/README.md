...
See https://github.com/mozilla/bigquery-etl/pull/1962

One-off query to stage a table adding first_seen_date and is_new_profile
to core_clients_daily:

```
CREATE OR REPLACE TABLE
  mozdata.tmp.klukas_core_cd_with_first_seen
PARTITION BY
  submission_date
CLUSTER BY
  is_new_profile,
...
(takes ~11 minutes to run, longest of these queries):

```
CREATE OR REPLACE TABLE
  mozdata.tmp.klukas_core_cls_with_first_seen
PARTITION BY
  submission_date
CLUSTER BY
  app_name,
...
Then, we can use the staged `klukas_core_cls_with_first_seen` to create
a table of just `new_profiles` aggregates that we'll later join with
the existing `firefox_nondesktop_exact_mau` data.
The query here copies in the definition of `nondesktop_clients_last_seen`,
but references our staging table for `core` pings:

```
CREATE OR REPLACE TABLE
  mozdata.tmp.klukas_nondesktop_new_profiles
PARTITION BY
  submission_date
AS
WITH glean_final AS (
...
```
CREATE OR REPLACE TABLE
  mozdata.tmp.klukas_nondesktop_exact_mau
PARTITION BY
  submission_date
AS
WITH base AS (
...
SELECT 
  *,
  ...
  distribution_id))) AS joinkey
FROM
  mozdata.tmp.klukas_nondesktop_new_profiles AS np
)
SELECT 
  base.* EXCEPT (joinkey),
  np.new_profiles
...
And finally stage the mobile_usage table:

```
CREATE OR REPLACE TABLE
  mozdata.tmp.klukas_mobile_usage_with_new_profiles
PARTITION BY
  submission_date
AS
WITH base AS (
...
```
# Have to explicitly delete this table due to change in clustering fields.
bq rm -f moz-fx-data-shared-prod:telemetry_derived.core_clients_daily_v1
bq cp mozdata:tmp.klukas_core_cd_with_first_seen moz-fx-data-shared-prod:telemetry_derived.core_clients_daily_v1

# Others can be copied into place without a delete.
bq cp -f mozdata:tmp.klukas_core_cls_with_first_seen moz-fx-data-shared-prod:telemetry_derived.core_clients_last_seen_v1
bq cp -f mozdata:tmp.klukas_nondesktop_exact_mau moz-fx-data-shared-prod:telemetry_derived.firefox_nondesktop_exact_mau28_v1
bq cp -f mozdata:tmp.klukas_mobile_usage_with_new_profiles moz-fx-data-shared-prod:telemetry_derived.mobile_usage_v1
```
...
And then we rerun tests on the PR to make sure dry runs all succeed, and merge.

We also manually rerun these jobs in Airflow now for the most recent date to make sure
no errors crop up.
