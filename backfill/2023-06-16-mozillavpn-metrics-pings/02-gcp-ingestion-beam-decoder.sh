...
PROJECT="moz-fx-data-backfill-3"
JOB_NAME="mozillavpn-metrics-backfill"
...
mvn \
  ...
  exec:java -Dexec.mainClass=com.mozilla.telemetry.Decoder -Dexec.args="\
      ...
      --jobName=$JOB_NAME \
      --project=$PROJECT \
      --geoCityDatabase=gs://moz-fx-data-prod-geoip/GeoIP2-City/20230616/GeoIP2-City.mmdb \
      --geoIspDatabase=gs://moz-fx-data-prod-geoip/GeoIP2-ISP/20230616/GeoIP2-ISP.mmdb \
      --geoCityFilter=gs://moz-fx-data-prod-dataflow-templates/cities15000.txt \
      --schemasLocation=gs://moz-fx-data-prod-dataflow/schemas/202306150145_ca90db03.tar.gz \
